...
#path_ult='D://UROP/millionsongsubset_full/MillionSongSubset'
#file = 'ultimate_csv_size.csv'
def pre_no_sound(file='ultimate_csv_size.csv', save_csv=False,
                 lastfm_db=None):
    '''
    Parameters
    ----------
    ...
    file: str
        The name of the input csv, default--ultimate_csv_size.csv generated by 
        mp3_length.py
        
    save_csv: bool
        If True, the output dataframe is saved as a csv
        
    lastfm_db: str
        If path is provided, it will read last_tags.db from the given path.
        
    
    Returns
    -------
    df: pd.DataFrame
        The dataframe that will be used for running no_sound(). The dataframe
        consist of tracks filtered by get_faulty_mp3() with at least one tag 
        provided by last_tags.db.
        
    '''
    if lastfm_db:
        db.set_path(lastfm_db)
        
    df_size = pd.read_csv(os.path.join(path_ult, file))

    df_test = db.get_tids_with_tag()
    df_test = pd.DataFrame(data={'tid': df_test})

    df_final = df_size.merge(df_test, left_on='track_id', right_on='tid')

    fault = get_faulty_mp3.get_faulty_mp3(file)

    df_final = df_final[-df_final.track_id.isin(fault)]
    ...
    if save_csv:
        df_final.to_csv(os.path.join(path_ult, 'pre_no_sound.csv'), index=False)
