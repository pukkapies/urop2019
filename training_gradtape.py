...
def train(train_dataset, valid_dataset, frontend, strategy, config, config_optim, epochs, resume_time=None, update_freq=1, analyse_trace=False):
    ...
    with strategy.scope():
        ...
        train_log_dir = os.path.join(path_logs, 'train/')
        train_summary_writer = tf.summary.create_file_writer(train_log_dir)
        
        if valid_dataset:
            val_log_dir = os.path.join(path_logs, 'validation/')
        ...
        if analyse_trace: # make sure the variable LD_LIBRARY_PATH is properly set up
            ...
            prof_log_dir = os.path.join(path_logs, 'profile/')
        ...
        for epoch in tf.range(prev_epoch+1, epochs, dtype=tf.int64):
            ...
            if valid_dataset:
                ...
                if (config.early_stop_min_d) or (config.early_stop_patience):
                    ...
                    if os.path.isfile(os.path.join(path_checkpoints, 'early_stopping.npy')):
                        cumerror = int(np.load(os.path.join(path_checkpoints, 'early_stopping.npy')))
                    
                    if val_metrics_2.result() > (max_metric + config.early_stop_min_d):
                        max_metric = val_metrics_2.result()
                        cumerror = 0
                        np.save(os.path.join(path_checkpoints, 'early_stopping.npy'), cumerror)
                    else:
                        cumerror += 1
            ...
            checkpoint_path = os.path.join(path_checkpoints+str(tf.constant(0).numpy()), 'epoch')
...
#          log_dir = '/srv/data/urop/log_aden/', path_checkpoints='/srv/data/urop/model_aden',
...
#         path_checkpoints, e.g. log-mel-spectrogram_20190823-000120, then checkpoint
